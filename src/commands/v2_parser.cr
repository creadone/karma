  ...
  module Commands
    private def self.parse_v2(object : Hash(String, JSON::Any)) : Directive
      ...
      directive = case op
                  ...
                  when "idempotency.prune"
                    Directive.new("idempotency_prune", before_unix: before_unix_from(object), limit: limit_from(object), token: token, protocol_version: 2_u32)
                  when "idempotency.snapshot_fetch_chunk"
                    Directive.new("idempotency_snapshot_fetch_chunk", cursor: offset_from(object), limit: limit_from(object), token: token, protocol_version: 2_u32)
                  when "snapshot.info"
      ...
      if object.has_key?("idempotency_key")
        directive.idempotency_key = idempotency_key_from(object)
        directive.fingerprint = fingerprint_from(object)
        directive.idempotency_created_at_unix = idempotency_created_at_unix_from(object)
      end
      directive
    end
  end
